...
async function insertTags(client, recipe_id, tags)
{
  ...
  const insertPlaceholders = uniqueTags.map((_, i) => `($${i + 1})`).join(', ');
  console.log("INSERT PLACEHOLDERS: ", insertPlaceholders);
...
async function createRecipe(recipe)
{
  ...
  try {
    ...
    const {name, description, user_id, image_url} = recipe;
    ...
    const {sql, values} = buildStepsInsert(recipe_id, recipe.steps);
    ...
    // Insert tags
    insertTags(client, recipe_id, recipe.tags);
    ...
    //   DO $$
    //   BEGIN
    //   RAISE EXCEPTION 'TEST_FAILURE: Intentionally stopping creation to test transaction rollback';
    //   END $$;
    // `);
